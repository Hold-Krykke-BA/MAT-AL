...
public class ManhattanProblem implements AStarProblem, Heuristic {
            ...
            for (int x = 0; x < mg.width(); ++x) {
                switch (mg.get(x, y)) {
                    case ManhattanGraph.EMPTY:
                        System.out.print(" " + writeChar(nodeCount) + " ");
                        ++nodeCount;
                        break;
    ...
    private String writeChar(int nodeCount) {
        String res = "" + nodeCount;
        if (nodeCount < 10) res = nodeCount + "  "; //[1  ]
        if (nodeCount >= 10 && nodeCount < 100) res = nodeCount + " "; //[22 ]
        return res;
    }
