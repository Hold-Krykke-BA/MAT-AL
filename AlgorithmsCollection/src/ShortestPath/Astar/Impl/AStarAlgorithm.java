...
public class AStarAlgorithm {
    ...
    public AStarAlgorithm(AStarProblem problem, int source, int end) {
        this.directedGraph = problem.getGraph();
        this.startNode = problem.getStartNode();
        this.targetNode = problem.getTargetNode();
        this.heuristic = problem.getHeuristic();

        heuristicArr = new float[directedGraph.V()];
        ...
        for (int i = 0; i < directedGraph.V(); i++) {
            ...
            pathArr[i] = -1;
            visitedArr[i] = false;
            heuristicArr[i] = heuristic.h(end, i);
        }
        weightArr[source] = 0;
        pq.add(source);
    ...
    class WeightComparator implements Comparator<Integer> {
        ...
        public int compare(Integer o1, Integer o2) {
            float f = (weightArr[o1] + heuristicArr[o1]) - (weightArr[o2] + heuristicArr[o2]);
            if (f > 0) return 1;
            if (f < 0) return -1;
            return 0;
        }
